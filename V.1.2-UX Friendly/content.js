...
let searchListDOM = document.getElementById("serach-list");
...
async function customSerach() {
  ...
  let numOfQuery = 5;
  let lang = "lang_en";
  let url = "https://www.googleapis.com/customsearch/v1?key=" + API_KEY + "&cx=" + SearchEngineID + "&alt=" + alt + "&num=" + numOfQuery + "&q=" + q + "&lr=" + lang;
    ...
    if (data.items && data.items.length > 0) {
      for (let index = 0; index < data.items.length; index++) {
        if(data.items[index].link.includes("en-US")) {
          let title = data.items[index].title.replace(/</g, '&lt;').replace(/>/g, '&gt;').split("|")[0];
          searchListDOM.innerHTML += `<li><a href="${data.items[index].link}" target="_blank">${title}</a></li>`;
        }
      }
    }
  }else {
